...
import it.fivano.symusic.core.ReleaseFromEuroadrenalineService;
...
public class EuroadrenalineServlet extends BaseAction {
...
//			urlSuccessivo += "&excludeRelaseRip="+flagRip;
			...
			request.setAttribute("urlPrecedente", " ");
			request.setAttribute("urlSuccessivo", " ");
			
			List<ReleaseModel> listRelease = new ArrayList<ReleaseModel>();
			ReleaseFromEuroadrenalineService scenelog = new ReleaseFromEuroadrenalineService(user.getId());
			scenelog.setEnableBeatportService(false);
			scenelog.setExcludeRipRelease(flagRip);
			
			for(String gen : genreList) {
				listRelease.addAll(scenelog.parsePresceneRelease(gen, numPagine));
			}
